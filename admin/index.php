...
?>
Add a Category  <? if ($name) echo 'for '.$name;?> <br/>
<br/><form action = "index.php?mainid=<?= $mainid;?>" method = "post">
Category name:<input type = "text" name = "subcatname" value = "<?=$subname;?>"><br/>
<input type = "hidden" name = "subcatid" value = "<?= $editsub;?>">
<input name ="submit" type = "submit" value = "<? if (!isset($editsub)) echo "Add Category"; else echo "Update Category";?>"/>
</form>
<?
if (isset($editsub))
   {
?>	   
<a href = "index.php?mainid=<?= $mainid;?>">Return to Add Categories</a>
<?
   }
   $dbh=null;
?>
